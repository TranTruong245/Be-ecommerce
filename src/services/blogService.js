		...
		try {
			...
			if (res.rows && res.rows.length > 0) {
				for (let i = 0; i < res.rows.length; i++) {
					res.rows[i].image = new Buffer(res.rows[i].image, 'base64').toString(
						'binary'
					);
					res.rows[i].userData = await db.User.findOne({
						where: { id: res.rows[i].userId },
					});
					res.rows[i].commentData = await db.Comment.findAll({
						where: { blogId: res.rows[i].id },
					});
				}
			}
		...
		try {
			...
			if (res && res.length > 0) {
				for (let i = 0; i < res.length; i++) {
					...
					res[i].commentData = await db.Comment.findAll({
						where: { blogId: res[i].id },
					});
				}
			}
