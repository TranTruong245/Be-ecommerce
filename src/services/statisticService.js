...
// }
let totalPriceDiscount = (price, discount) => {
	// Ép kiểu price và các giá trị liên quan đến số học thành số thực
	price = parseFloat(price);
	let value = parseFloat(discount.voucherData.typeVoucherOfVoucherData.value);
	let maxValue = parseFloat(
		discount.voucherData.typeVoucherOfVoucherData.maxValue
	);

	if (discount.voucherData.typeVoucherOfVoucherData.typeVoucher === 'percent') {
		if ((price * value) / 100 > maxValue) {
		...
		try {
			if (!data.year) {
				...
				for (let i = 0; i < orderProduct.length; i++) {
					...
					for (let j = 0; j < orderProduct[i].orderDetail.length; j++) {
						totalprice +=
							parseFloat(orderProduct[i].orderDetail[j].realPrice) *
							parseFloat(orderProduct[i].orderDetail[j].quantity);
					}

					if (orderProduct[i].voucherId) {
						orderProduct[i].totalpriceProduct =
							parseFloat(totalPriceDiscount(totalprice, orderProduct[i])) +
							parseFloat(orderProduct[i].typeShipData.price);
					} else {
						orderProduct[i].totalpriceProduct =
							totalprice + parseFloat(orderProduct[i].typeShipData.price);
					}
				}
				...
				for (let i = 1; i <= 12; i++) {
					...
					for (let j = 0; j < orderProduct.length; j++) {
						if (
							...
							price += parseFloat(orderProduct[j].totalpriceProduct);
						}
					}
					arrayMonthValue.push(price);
				}
					...
					for (let j = 0; j < orderProduct[i].orderDetail.length; j++) {
						...
						for (let k = 0; k < receiptDetail.length; k++) {
							avgPrice +=
								...
								parseFloat(receiptDetail[k].price);
